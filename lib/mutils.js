...
class MUtils {
    ...
    static makeWhenString(weekday, day, month, year, hours, minutes) {
        ...
        let dateObj, weekday_, monthDay, monthString, yearString;
        let result;

        if(year) {
            dateObj = new Date(year, month, day);
            weekday_ = constants.WEEKDAYS[dateObj.getDay()];
            ...
            result = weekday_+" "+monthDay+" "+monthString+" "+yearString+
                " at "+fourFigureTime;
        } else {
            result = "Every "+weekday+" at "+fourFigureTime;
        }
