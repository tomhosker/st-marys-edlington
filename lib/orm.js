...
class OtherDocumentsORM extends SuperORM {
    ...
    fetchOtherDocuments() {
        const queryString =
            "SELECT * FROM OtherDocument ORDER BY date_published;";
...
class DeleteFromOtherDocumentORM extends SuperORM {
    ...
    wrapUp() {
        const properties = {
            ...
            formAction: this.action,
            otherDocumentSummaries:
                makeOtherDocumentSummaries(this.data.otherDocuments)
        };

        this.finaliser.protoRender(
            this.req, this.res, "deletefromOtherDocument", properties
        );
    }
}
