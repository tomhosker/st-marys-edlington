...
function makeServiceSummaries(columns, rows) {
    ...
    for(let i = 0; i < rows.length; i++) {
        ...
        serviceSummary =
            { id: objectifiedRow.id, description: description };
...
function makeDescription(dataObj) {
    ...
    const when =
        MUtils.makeWhenString(
            dataObj.weekday, dataObj.day, dataObj.month,
            dataObj.year, dataObj.hours, dataObj.minutes
        );
