...
class Uploader {
    ...
    insertNewServiceTime(req, res, tableName) {
        const rawParams = [
            ...
            MUtils.nullify(req.body.remarks)
        ];
        const query =
            "INSERT INTO "+tableName+" (weekday, hours, minutes, "+
                "location, service_type, day, month, year, remarks) " +
        ...
        const checkDateConfig = MUtils.xor(
            (req.body.weekday === constants.NULL_WEEKDAY),
            (req.body.date === "")
        );
        let properties;

console.log(req.body.weekday === constants.NULL_WEEKDAY);
console.log(req.body.date === "");
        ...
        if(checkDateConfig) {
            ...
            properties = {
                ...
                success: false,
                error: "You must specify EITHER a weekday or a date, but "+
...
function getHoursFrom24HourTime(timeString) {
    const stringRep =
        timeString.slice(EXPECTED_HOURS_START, EXPECTED_HOURS_STOP);
...
function getMinutesFrom24HourTime(timeString) {
    const stringRep =
        timeString.slice(EXPECTED_MINS_START, EXPECTED_MINS_STOP);
    const result = parseInt(stringRep);
