...
class Scraper
{
    ...
    fetchAsIsPart2(req, res, tableName)
    {
        ...
        client.query(queryString, (err, result) => {
            ...
            that.finaliser.protoRender(
                req, res, "asis", { title: tableName, data: data });
        });
    }
...
class IndexScraper extends SuperScraper
{
    ...
    {
        const queryString =
            "SELECT weekday, hours, minutes, length_in_minutes, "+
                "service_type, RealWorldAddress.short_name AS "+
                "address_name "+
            "FROM ServiceTime "+
    ...
    {
        const queryString =
            "SELECT Contact.*, RealWorldAddress.short_name AS address "+
            "FROM ParishRole "+
    ...
    wrapUp()
    {
        let maker = new IndexMaker(this.data);
