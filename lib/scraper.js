...
/*        const queryString =
        ...
        const queryString = "SELECT Contact.* "+"FROM ParishRole JOIN Contact ON Contact.code = ParishRole.contact WHERE ParishRole.code = 'computer-guy';";
        let that = this;
        let extract;
...
console.log(this.data.webmaster);

        if(this.data.webmaster.length === 1) {
            rowObj = MUtils.objectifyRow(this.data.webmaster[0]);
...
function makeNewsletters(data) {
    ...
    for(let i = 0; i < data.rows.length; i++) {
        ...
        newsletter =
            { weekBeginning: weekBeginning, link: objectifiedRow.link };
